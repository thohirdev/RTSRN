...
class TextBase(object):
    ...
    def CRNN_init(self, recognizer_path=None, opt=None):
        ...
        # Hitung kompleksitas (Opsional, tapi bagus untuk log)
        try:
        ...
        # --- MODIFIKASI BYPASS (INTJ FIX) ---
        # Cek apakah path kosong. Jika ya, pakai random weights.
        if model_path is None or model_path == '':
            print("WARNING: Path CRNN kosong! Menggunakan Random Weights untuk Testing (Bypass).")
            # Tidak melakukan torch.load, model tetap menggunakan inisialisasi awal (random)
        else:
            # Kode asli dijalankan HANYA jika path valid
            if os.path.exists(model_path):
                ...
                print(f"WARNING: File {model_path} tidak ditemukan! Menggunakan Random Weights.")
        # ------------------------------------
