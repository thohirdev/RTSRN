...
class TextSR(base.TextBase):
                    ...
                    if self.args.arch == 'rtsrn':
                        ...
                        for i in range(self.args.stu_iter):
                            ...
                            if self.args.arch in ['rtsrn']:
                                ...
                                # B
                                # lengths_input = torch.zeros(...) # <-- Matikan ini
                                
                                # Bypass pemanggilan .lm()
                                lm_out = None 
                                
                                # if self.config.TRAIN.ngpu>1:     # <-- Matikan blok ini
                                ...
                                # readed_txts = self.converter_crnn.decode(...) # <-- Boleh dimatikan atau dibiarkan jika lm_input ada
                                
                                # Matikan lm_txts karena butuh lm_out
                                ...
                                lm_txts = [] # Ganti dengan list kosong
                                visual_clues = None
                                ...
                                # KODE PENGGANTI:
                                fsup_sem_loss = torch.tensor(0.0).to(images_hr.device) # Set loss jadi 0
                                # ------------------------------------
            ...
            ##############################################################
                ...
                if self.args.random_reso:
                    ...
                    for i in range(self.args.stu_iter):
                        ...
                        if self.args.arch in ['rtsrn']:
                            # --- MODIFIKASI BYPASS EVAL (INTJ FIX) ---
                            # Matikan pemanggilan LM (Language Model) di fase evaluasi
                            
                            # generate linguistical clue
                            lm_input = label_vecs_final.clone().squeeze(2).detach().transpose(1,2)
                            
                            # lengths_input = ... (Matikan ini)
                            
                            # Bypass pemanggilan model .lm
                            lm_out = None 
                            
                            # KODE ASLI YANG DIMATIKAN:
                            ...
                            # visual_clues logic sudah dimatikan di kode asli, biarkan None
                            visual_clues = None
                            
                            # PENTING: Matikan juga baris ini agar tidak error NoneType
    ...
    def demo(self):
        # --- FIX 1: IMPORT LIBRARY DISINI AGAR TIDAK ERROR ---
        import torchvision 
        # -----------------------------------------------------
        ...
        # Buat folder output
        output_dir = './demo_result_images'
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)

        print(f"Memproses gambar dari folder: {self.args.demo_dir}")

        for im_name in tqdm(os.listdir(self.args.demo_dir)):
            # --- FIX 2: TOLAK FILE SAMPAH MAC (._) ---
            if im_name.startswith('.'): 
                continue 
            
            if not im_name.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
                continue
            # -----------------------------------------
            ...
            # --- Bagian Recognition (Biarkan saja) ---
            pred_str_lr = [""]
            pred_str_sr = [""]
            ...
            if self.args.rec == 'moran':
                ...
                moran_output_lr = moran(moran_input_lr[0], moran_input_lr[1], moran_input_lr[2], moran_input_lr[3], test=True, debug=True)
            ...
            try:
                ...
                img_lr_save = images_lr[:, :3, :, :].cpu()
                img_sr_save = images_sr[:, :3, :, :].cpu()
                
                # Resize LR biar tingginya sama kayak SR
                img_lr_save = torch.nn.functional.interpolate(img_lr_save, size=(32, 128), mode='bicubic')
